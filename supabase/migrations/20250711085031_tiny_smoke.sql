...
-- Allow authenticated users to SELECT, UPDATE, DELETE customers
CREATE POLICY "Authenticated users can read_update_delete customers"
  ON customers
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Authenticated users can update_delete customers"
  ON customers
  FOR UPDATE, DELETE
  TO authenticated
  USING (true);
