...
const PackageCard: React.FC<PackageCardProps> = ({ package: pkg, onSelect }) => {
  return (
    <div className={`bg-white rounded-lg border-2 p-4 shadow-sm hover:shadow-md transition-all ${
      pkg.popular ? 'border-loop-secondary ring-2 ring-loop-secondary ring-opacity-20' : 'border-gray-200'
    }`}>
      {pkg.popular && (
      ...
      <div className="text-center mb-3">
        ...
        <p className="text-2xl font-bold text-loop-primary">{pkg.price}</p>
